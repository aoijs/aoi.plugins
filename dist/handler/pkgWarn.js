...
async function pkgWarn() {
    try {
        ...
        if (!isDevVersion && version !== latestVersion) {
            console.warn(`\x1b[31m@akarui/aoi.plugins:\x1B[0m ${pack.name} is outdated! Update with "npm install ${pack.name}@latest".`);
        }
        else if (isDevVersion) {
            console.warn("\x1b[31m@akarui/aoi.plugins:\x1B[0m You are currently on a development version.");
        }
    }
    catch (error) {
        console.warn(`\x1b[31m@akarui/aoi.plugins:\x1B[0m Failed to check for updates: ${error}`);
    }
}
